  ...
  function seedData() {
    return {
      ...
      users: [
        ...
        { id: "u-worker-1", name: "Trabalhador 1", email: "trabalhador@sb.com", phone: "939000003", password: "Trab12345", role: "trabalhador", department: "Secretaria Academica" },
        { id: "u-user-1", name: "Visitante 1", email: "usuario@sb.com", phone: "939000004", password: "User12345", role: "usuario" }
      ],
      queue: [],
      history: []
    };
    ...
    try {
      ...
      parsed.users = parsed.users.map((user) => {
        if (!user.phone) {
          ...
          if (user.email === "admin2@sb.com") user.phone = "939000002";
          if (user.email === "trabalhador@sb.com") user.phone = "939000003";
          if (user.email === "usuario@sb.com") user.phone = "939000004";
        }
  ...
  const store = {
    ...
    login(identifier, password, selectedRole, loginMethod) {
      ...
      const user = data.users.find((item) => {
        if (loginMethod === "telefone") return item.phone && item.phone === normalizedPhone;
        if (loginMethod === "email") return item.email === normalizedEmail;
        return item.email === normalizedEmail || (item.phone && item.phone === normalizedPhone);
      });
    ...
    register(payload) {
      ...
      if (phone && phone.length < 8) return { ok: false, message: "Telefone invalido." };
      ...
      return updateData((data) => {
        ...
        if (phone && data.users.some((item) => item.phone === phone)) {
          return { ok: false, message: "Ja existe conta com este telefone." };
        }
    ...
    continueAsGuest() {
      ...
      setSession({
        email: `visitante-${stamp}@guest.local`,
        phone: "",
        name: "Visitante sem conta",
        role: "usuario",
        isGuest: true
      });

      return { ok: true, redirect: "visitante.html" };
    },
