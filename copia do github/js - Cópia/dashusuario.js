(function () {
  ...
  if (dadoEmail) dadoEmail.textContent = session.isGuest ? "Sem conta" : (session.email || "-");
  if (dadoPerfil) dadoPerfil.textContent = "visitante";
  if (dadoLogin) dadoLogin.textContent = formatDate(session.loggedAt);
  if (notificationEmailInput) notificationEmailInput.value = session.isGuest ? "" : (session.email || "");
