...
function _renderModalDetalhesAdmin(s, numero, servico) {
  ...
  window.abrirDocumentoAtendimento = function () {
    ...
    if (nomeFich && ficBloco && semFich) {
      ...
      if (btnDl) {
        btnDl.href =
          `${BASE()}/senhas/${senhaAtual.id}/ficheiro`;
  ...
  window.removerTrabalhador = async function (id, nome) {
    ...
    if (window.UX04) {
      confirmado = await UX04.confirm({
        titulo:      `Desactivar "${nome}"?`,
        mensagem:    'O membro não será apagado — a conta ficará inactiva. Pode ser reactivada pelo administrador.',
        perigo:      true,
        txtConfirmar:'Desactivar',
        txtCancelar: 'Cancelar',
        emoji:       '⚠️',
      });
    } else {
      confirmado = window.confirm(`Desactivar "${nome}"?`);
    }
    ...
    try {
      ...
      if (r.ok) {
        if (window.UX04) {
          UX04.toast(`"${nome}" desactivado com sucesso.`, 'success');
        } else {
          showToast(`"${nome}" desactivado.`, 'success');
        }

        await carregarTrabalhadores();

      } else {
        const msg = d.erro || 'Não foi possível desactivar.';

        if (window.UX04) {
          UX04.toast(msg, 'error');
        } else {
