...
// PR-7: impedir chamadas durante pausa
  ...
  window.denyCurrentTicket = async function () {
    ...
    // ─────────────────────────────────────────
    // FIX UX-03
    // senhaId deve existir antes do lock
    // ─────────────────────────────────────────
    const senhaId = document.getElementById("currentSenhaId")?.value;
    ...
    if (window.UX04) {
      ...
      motivo = await UX04.prompt({
        titulo: `Negar Senha ${senhaAtual.numero}`,
        subtitulo: `Serviço: ${senhaAtual.servico?.nome || '—'}`,
        emoji: '🚫',
        perigo: true,
        ...
        txtConfirmar: 'Negar Senha',
        txtCancelar: 'Cancelar',

        minLength: 3,
        maxLength: 200,

        errMsg: 'Indique um motivo válido.'
      });

    } else {

      motivo = window.prompt(
        `Motivo da negação da senha ${senhaAtual.numero}:`,
        ""
      );
    ...
    if (!motivo.trim()) {
      N && N.notify('warn', 'É necessário indicar o motivo.');
      return;
    }
    ...
    if (window.UX03) {
      window.UX03.btnLoading(btn, true, 'A negar...');
    }
    ...
    try {
      ...
      console.error("[denyCurrentTicket]", err);
      ...
      if (window.UX04) {
        ...
        UX04.toast(
          err.message || "Erro ao negar senha.",
          'error'
        );
  ...
  window.confirmarRedirecionamento = async function () {
    ...
    if (window.UX03) window.UX03.btnLoading(btnConfirmar, true, 'A redirecionar…');
    ...
    try {
      ...
      if (!resp.ok) {
        const errMsg = data.erro || 'Erro ao redirecionar.';
        if (msgEl) { msgEl.textContent = errMsg; msgEl.style.color = '#dc2626'; }
        if (window.UX04) UX04.toast(errMsg, 'error');
        return;
      }
      ...
      if (window.UX04) {
        UX04.toast('Erro de ligação ao servidor.', 'error');
      } else if (msgEl) {
  ...
  window.abrirDocumentoAtendimento = function () {
    ...
    const nomeFich =
      partes.find(p => p.startsWith("FICHEIRO:"))
        ?.replace("FICHEIRO:", "")
        .trim() || null;

    const linhasForm =
      partes.filter(p => !p.startsWith("FICHEIRO:")).join("\n");
  ...
  window.sair = async function () {
    ...
    if (window.UX04) {
      confirmado = await UX04.confirm({
        ...
        mensagem:    'Tem a certeza que deseja sair do sistema? A sessão actual será encerrada.',
        txtConfirmar:'Sair',
        txtCancelar: 'Continuar aqui',
        aviso:       true,
        emoji:       '🚪',
      });
    } else {
